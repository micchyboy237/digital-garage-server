...
import { client as s3Client } from "../../../../../../2024BoilerplateBaseTemplates/aws/aws-doc-sdk-examples/javascriptv3/example_code/s3/client.js"
...
export async function legallyEmptyAndDeleteBuckets(bucketNames) {
  for (const bucketName of bucketNames) {
    const objectsResponse = await s3Client.send(new ListObjectVersionsCommand({ Bucket: bucketName }))
    ...
    for (const version of objectsResponse.Versions || []) {
      ...
      try {
        ...
        console.log(`Unable to fetch legal hold for ${Key} in ${bucketName}: '${err.message}'`)
      }
      ...
      try {
        ...
        console.log(`Unable to fetch object lock retention for ${Key} in ${bucketName}: '${err.message}'`)
      }

      await s3Client.send(new DeleteObjectCommand({ Bucket: bucketName, Key, VersionId }))
    }
