...
import { AuthFindFirst } from "./AuthFindFirst"
import { AuthFindMany } from "./AuthFindMany"
import { AuthFindUnique } from "./AuthFindUnique"
import { AuthCreate } from "./AuthCreate"
import { AuthCreateMany } from "./AuthCreateMany"
import { AuthUpdate } from "./AuthUpdate"
import { AuthUpdateMany } from "./AuthUpdateMany"
import { AuthUpsert } from "./AuthUpsert"
import { AuthDelete } from "./AuthDelete"
import { AuthDeleteMany } from "./AuthDeleteMany"
import { AuthAggregate } from "./AuthAggregate"
import { AuthCount } from "./AuthCount"
import { AuthGroupBy } from "./AuthGroupBy"
...
/**
 * Generates an Express router for Auth model.
 * @param config Contains optional middleware to enable routes.
 * @param customUrlPrefix Optional custom URL prefix for the routes.
 * @returns {express.Router}
 */
export function AuthRouter(config: RouteConfig<RequestHandler>) {
  const router = express.Router()
  const basePath = (config.customUrlPrefix || "") + (config.addModelPrefix ? "/auth" : "")
  ...
  if (config.enableAll || config?.findFirst) {
    ...
    setupRoute("/first", "get", before, AuthFindFirst as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.findMany) {
    ...
    setupRoute("/", "get", before, AuthFindMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.findUnique) {
    ...
    setupRoute("/:id", "get", before, AuthFindUnique as any, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.create) {
    ...
    setupRoute("/", "post", before, AuthCreate as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.createMany) {
    ...
    setupRoute("/many", "post", before, AuthCreateMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.update) {
    ...
    setupRoute("/", "put", before, AuthUpdate as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.updateMany) {
    ...
    setupRoute("/many", "put", before, AuthUpdateMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/many", ...after)
    }
  }
  ...
  if (config.enableAll || config?.upsert) {
    ...
    setupRoute("/", "patch", before, AuthUpsert as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.delete) {
    ...
    setupRoute("/", "delete", before, AuthDelete as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.deleteMany) {
    ...
    setupRoute("/many", "delete", before, AuthDeleteMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/many", ...after)
    }
  }
  ...
  if (config.enableAll || config?.aggregate) {
    ...
    setupRoute("/aggregate", "get", before, AuthAggregate as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.count) {
    ...
    setupRoute("/count", "get", before, AuthCount as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.groupBy) {
    ...
    setupRoute("/groupby", "get", before, AuthGroupBy as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
