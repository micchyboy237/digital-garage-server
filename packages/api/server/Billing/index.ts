...
import { BillingFindFirst } from "./BillingFindFirst"
import { BillingFindMany } from "./BillingFindMany"
import { BillingFindUnique } from "./BillingFindUnique"
import { BillingCreate } from "./BillingCreate"
import { BillingCreateMany } from "./BillingCreateMany"
import { BillingUpdate } from "./BillingUpdate"
import { BillingUpdateMany } from "./BillingUpdateMany"
import { BillingUpsert } from "./BillingUpsert"
import { BillingDelete } from "./BillingDelete"
import { BillingDeleteMany } from "./BillingDeleteMany"
import { BillingAggregate } from "./BillingAggregate"
import { BillingCount } from "./BillingCount"
import { BillingGroupBy } from "./BillingGroupBy"
...
/**
 * Generates an Express router for Billing model.
 * @param config Contains optional middleware to enable routes.
 * @param customUrlPrefix Optional custom URL prefix for the routes.
 * @returns {express.Router}
 */
export function BillingRouter(config: RouteConfig<RequestHandler>) {
  const router = express.Router()
  const basePath = (config.customUrlPrefix || "") + (config.addModelPrefix ? "/billing" : "")
  ...
  if (config.enableAll || config?.findFirst) {
    ...
    setupRoute("/first", "get", before, BillingFindFirst as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.findMany) {
    ...
    setupRoute("/", "get", before, BillingFindMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.findUnique) {
    ...
    setupRoute("/:id", "get", before, BillingFindUnique as any, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.create) {
    ...
    setupRoute("/", "post", before, BillingCreate as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.createMany) {
    ...
    setupRoute("/many", "post", before, BillingCreateMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.update) {
    ...
    setupRoute("/", "put", before, BillingUpdate as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.updateMany) {
    ...
    setupRoute("/many", "put", before, BillingUpdateMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/many", ...after)
    }
  }
  ...
  if (config.enableAll || config?.upsert) {
    ...
    setupRoute("/", "patch", before, BillingUpsert as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.delete) {
    ...
    setupRoute("/", "delete", before, BillingDelete as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.deleteMany) {
    ...
    setupRoute("/many", "delete", before, BillingDeleteMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/many", ...after)
    }
  }
  ...
  if (config.enableAll || config?.aggregate) {
    ...
    setupRoute("/aggregate", "get", before, BillingAggregate as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.count) {
    ...
    setupRoute("/count", "get", before, BillingCount as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.groupBy) {
    ...
    setupRoute("/groupby", "get", before, BillingGroupBy as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
