...
import { NotificationFindFirst } from "./NotificationFindFirst"
import { NotificationFindMany } from "./NotificationFindMany"
import { NotificationFindUnique } from "./NotificationFindUnique"
import { NotificationCreate } from "./NotificationCreate"
import { NotificationCreateMany } from "./NotificationCreateMany"
import { NotificationUpdate } from "./NotificationUpdate"
import { NotificationUpdateMany } from "./NotificationUpdateMany"
import { NotificationUpsert } from "./NotificationUpsert"
import { NotificationDelete } from "./NotificationDelete"
import { NotificationDeleteMany } from "./NotificationDeleteMany"
import { NotificationAggregate } from "./NotificationAggregate"
import { NotificationCount } from "./NotificationCount"
import { NotificationGroupBy } from "./NotificationGroupBy"
...
/**
 * Generates an Express router for Notification model.
 * @param config Contains optional middleware to enable routes.
 * @param customUrlPrefix Optional custom URL prefix for the routes.
 * @returns {express.Router}
 */
export function NotificationRouter(config: RouteConfig<RequestHandler>) {
  const router = express.Router()
  const basePath = (config.customUrlPrefix || "") + (config.addModelPrefix ? "/notification" : "")
  ...
  if (config.enableAll || config?.findFirst) {
    ...
    setupRoute("/first", "get", before, NotificationFindFirst as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.findMany) {
    ...
    setupRoute("/", "get", before, NotificationFindMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.findUnique) {
    ...
    setupRoute("/:id", "get", before, NotificationFindUnique as any, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.create) {
    ...
    setupRoute("/", "post", before, NotificationCreate as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.createMany) {
    ...
    setupRoute("/many", "post", before, NotificationCreateMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.update) {
    ...
    setupRoute("/", "put", before, NotificationUpdate as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.updateMany) {
    ...
    setupRoute("/many", "put", before, NotificationUpdateMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/many", ...after)
    }
  }
  ...
  if (config.enableAll || config?.upsert) {
    ...
    setupRoute("/", "patch", before, NotificationUpsert as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.delete) {
    ...
    setupRoute("/", "delete", before, NotificationDelete as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.deleteMany) {
    ...
    setupRoute("/many", "delete", before, NotificationDeleteMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/many", ...after)
    }
  }
  ...
  if (config.enableAll || config?.aggregate) {
    ...
    setupRoute("/aggregate", "get", before, NotificationAggregate as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.count) {
    ...
    setupRoute("/count", "get", before, NotificationCount as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.groupBy) {
    ...
    setupRoute("/groupby", "get", before, NotificationGroupBy as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
