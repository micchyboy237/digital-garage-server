...
import { VehicleFindFirst } from './VehicleFindFirst';
import { VehicleFindMany } from './VehicleFindMany';
import { VehicleFindUnique } from './VehicleFindUnique';
import { VehicleCreate } from './VehicleCreate';
import { VehicleCreateMany } from './VehicleCreateMany';
import { VehicleUpdate } from './VehicleUpdate';
import { VehicleUpdateMany } from './VehicleUpdateMany';
import { VehicleUpsert } from './VehicleUpsert';
import { VehicleDelete } from './VehicleDelete';
import { VehicleDeleteMany } from './VehicleDeleteMany';
import { VehicleAggregate } from './VehicleAggregate';
import { VehicleCount } from './VehicleCount';
import { VehicleGroupBy } from './VehicleGroupBy';
...
/**
 * Generates an Express router for Vehicle model.
 * @param config Contains optional middleware to enable routes.
 * @param customUrlPrefix Optional custom URL prefix for the routes.
 * @returns {express.Router}
 */
export function VehicleRouter(config: RouteConfig<RequestHandler>) {
  const router = express.Router();
  const basePath = removeTrailingSlash(config.customUrlPrefix || '') +
                  removeTrailingSlash(config.addModelPrefix !== false ? '/vehicle' : '');
  ...
  if (config.enableAll || config?.findFirst) {
    ...
    setupRoute('/first', 'get', before, VehicleFindFirst as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.findMany) {
    ...
    setupRoute('/', 'get', before, VehicleFindMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.findUnique) {
    ...
    setupRoute('/:id', 'get', before, VehicleFindUnique as any, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.create) {
    ...
    setupRoute('/', 'post', before, VehicleCreate as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.createMany) {
    ...
    setupRoute('/many', 'post', before, VehicleCreateMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.update) {
    ...
    setupRoute('/', 'put', before, VehicleUpdate as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.updateMany) {
    ...
    setupRoute('/many', 'put', before, VehicleUpdateMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath) + '/many', ...after);
    }
  }
  ...
  if (config.enableAll || config?.upsert) {
    ...
    setupRoute('/', 'patch', before, VehicleUpsert as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.delete) {
    ...
    setupRoute('/', 'delete', before, VehicleDelete as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.deleteMany) {
    ...
    setupRoute('/many', 'delete', before, VehicleDeleteMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath) + '/many', ...after);
    }
  }
  ...
  if (config.enableAll || config?.aggregate) {
    ...
    setupRoute('/aggregate', 'get', before, VehicleAggregate as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.count) {
    ...
    setupRoute('/count', 'get', before, VehicleCount as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.groupBy) {
    ...
    setupRoute('/groupby', 'get', before, VehicleGroupBy as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
