...
import { SessionFindFirst } from "./SessionFindFirst"
import { SessionFindMany } from "./SessionFindMany"
import { SessionFindUnique } from "./SessionFindUnique"
import { SessionCreate } from "./SessionCreate"
import { SessionCreateMany } from "./SessionCreateMany"
import { SessionUpdate } from "./SessionUpdate"
import { SessionUpdateMany } from "./SessionUpdateMany"
import { SessionUpsert } from "./SessionUpsert"
import { SessionDelete } from "./SessionDelete"
import { SessionDeleteMany } from "./SessionDeleteMany"
...
/**
 * Generates an Express router for Session model.
 * @param config Contains optional middleware to enable routes.
 * @param customUrlPrefix Optional custom URL prefix for the routes.
 * @returns {express.Router}
 */
export function SessionRouter(config: RouteConfig<RequestHandler>) {
  const router = express.Router()
  const basePath = (config.customUrlPrefix || "") + (config.addModelPrefix ? "/session" : "")
  ...
  if (config.enableAll || config?.findFirst) {
    ...
    setupRoute("/first", "get", before, SessionFindFirst as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.findMany) {
    ...
    setupRoute("/", "get", before, SessionFindMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.findUnique) {
    ...
    setupRoute("/:id", "get", before, SessionFindUnique as any, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.create) {
    ...
    setupRoute("/", "post", before, SessionCreate as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.createMany) {
    ...
    setupRoute("/many", "post", before, SessionCreateMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.update) {
    ...
    setupRoute("/", "put", before, SessionUpdate as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.updateMany) {
    ...
    setupRoute("/many", "put", before, SessionUpdateMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/many", ...after)
    }
  }
  ...
  if (config.enableAll || config?.upsert) {
    ...
    setupRoute("/", "patch", before, SessionUpsert as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.delete) {
    ...
    setupRoute("/", "delete", before, SessionDelete as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/", ...after)
    }
  }
  ...
  if (config.enableAll || config?.deleteMany) {
    ...
    setupRoute("/many", "delete", before, SessionDeleteMany as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
      router.use(basePath + "/many", ...after)
    }
  }
  ...
  if (config.enableAll || config?.aggregate) {
    ...
    setupRoute("/aggregate", "get", before, SessionAggregate as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.count) {
    ...
    setupRoute("/count", "get", before, SessionCount as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
  ...
  if (config.enableAll || config?.groupBy) {
    ...
    setupRoute("/groupby", "get", before, SessionGroupBy as RequestHandler, inputValidator, outputValidator)
    if (after.length) {
