...
def find_files(base_dir, include, exclude):
    ...
    for root, dirs, files in os.walk(base_dir):
        ...
        for file in files:
            ...
            # Ensure ./package.json is matched only in the current directory
            is_current_package_json = (
                file_path == "package.json" and "./package.json" in include and root == base_dir)
            if (is_current_package_json or any(fnmatch.fnmatch(file_path, pat) for pat in include)) and not any(fnmatch.fnmatch(file_path, pat) for pat in exclude):
                matched_files.append(file_path)
